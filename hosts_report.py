...
file_object.write("Hostname,ip_list,Last Seen,First Seen,Platform,OS Version,OS Build,OS Product Name,Kernel Version,Model,Manufacturer,Type,Chassis,Last Reboot,Prevention Policy,Response Policy,Sensor Update Policy,Host ID,MAC Address,Connection MAC Address,Status,CPUID,Serial Number,Sensor Version,Sensor Tags\n")
offset = ''
...
while total > 0:
    ...
    for detail in detail_response["body"]["resources"]:
        ...
        print("Hosts Processed: ", host_iter)
        aid = get_detail(detail, "device_id")
        iplist = ""
        qh = falcon.command("query_hosts", filter="aid:'" + aid + "'")
        #code.interact(local=dict(globals(), **locals()))
        if qh["body"]["resources"]:
            gh = falcon.command("get_hosts", ids=qh["body"]["resources"])
            if gh["body"]["resources"][0]:
                data = gh["body"]["resources"][0]
                if data["network_interfaces"]:
                    for network_interface in data["network_interfaces"]:
                        iplist = iplist + network_interface["local_ip"] + "|"
                    iplist = iplist[:-1]
        ...
        file_object.write(hostname + "," + iplist + "," + last_seen + "," + first_seen + "," + platform + "," + os_version + "," + os_build + "," + os_product_name + "," + kernel_version + "," + model + "," + manufacturer + "," + type + "," + chassis + "," + last_reboot + "," + prevention_policy + "," + response_policy + "," + sensor_update_policy + "," + host_id + "," + mac_address + "," + connection_mac_address + "," + status + "," + cpuid + "," + serial_number + "," + sensor_version + "," + sensor_tags + "\n")
